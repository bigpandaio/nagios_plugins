...
def usage():
    ...
    print '\t--reverse\t\t\tReverse Alert - only if set to reverse,'
    ...
    print '\t--user\t\t\tUser name to conncet with,'
    print '\t--psas\t\t\tPassword to conncet with,'


def pull_graphite_data(user, password, url):
...
def get_hw_value(user, password, url, seconds=0):
    """Get the Holt-Winters value from a Graphite graph"""

    data = pull_graphite_data(user, password, url)
...
def get_value(user, password, url, seconds=0):
...
def main(argv):
    try:
        opts, args = getopt.getopt(argv, 'hWULru:c:w:s:',
                                    ['help', 'holt-winters', 'critupper',
                                     ...
                                     'seconds=', 'd1=', 'd2=', "user=", "pass=", "reverse="])
    ...
    if hw:
        graphite_data, graphite_lower, graphite_upper = get_hw_value(user, password, url, seconds)
        ...
        graphite_data1 = get_value(user, password, diff1, seconds)
        graphite_data2 = get_value(user, password, diff2, seconds)
        graphite_data = abs(graphite_data1 - graphite_data2)
    else:
        graphite_data = get_value(user, password, url, seconds)
